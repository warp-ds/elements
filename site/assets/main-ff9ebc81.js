 ...
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */let xr=class extends Br{constructor(o){if(super(o),this.et=j,o.type!==Nr.CHILD)throw Error(this.constructor.directiveName+"() can only be used in child bindings")}render(o){if(o===j||o==null)return this.ft=void 0,this.et=o;if(o===U)return o;if(typeof o!="string")throw Error(this.constructor.directiveName+"() called with a non-string value");if(o===this.et)return this.ft;this.et=o;const e=[o];return e.raw=e,this.ft={_$litType$:this.constructor.resultType,strings:e,values:[]}}};xr.directiveName="unsafeHTML",xr.resultType=1;const Dr=Pr(xr);function mt(r){return r&&r.__esModule&&Object.prototype.hasOwnProperty.call(r,"default")?r.default:r}const ce=String.raw`[A-Za-z][^/\s>]*`,kt=String.raw`(?<!\w)"(?:\\[^<>\n]|[^\\"<>\n])*"(?!\w)`,ft=String.raw`(?<!\w)'(?:\\[^<>\n]|[^\\'<>\n])*'(?!\w)`,Me=String.raw`${kt}|${ft}`,$t=String.raw`"(?<quotedAttrValue>[^"]*)"`,yt=String.raw`'(?<singleQuotedAttrValue>[^']*)'`,zt=String.raw`(?<unquotedAttrValue>[^\s"'\`=<>]+)`,_t=String.raw`[^=\s>/"']+(?=[=>\s]|$)`,qt=String.raw`${$t}|${yt}|${zt}`,Xt=String.raw`(?<attrName>${_t})(?:\s*=\s*(?:${qt}))?`,Yt=String.raw`${Me}|[^\s>]*[^\s>/]|[^\s>]*/(?!\s*>)`,Te=String.raw`(?<attrSpace>\s*)(?:${Xt}|(?<attrText>${Yt}))`,jt={comment:String.raw`<!--.*?-->`,dtd:String.raw`<![^>]+>`,startTag:String.raw`<(?<startTagName>${ce})(?<attrs>(?:${Te})*)\s*(?<closingSlash>/?)\s*>`,endTag:String.raw`</(?<endTagName>${ce})\s*>`,space:String.raw`\s+`,text:String.raw`[^<\s"']+|${Me}|['"]`,wildcard:String.raw`.`},Zt=Object.entries(jt).map(([r,o])=>`(?<${r}>${o})`).join("|");function*St(r,o){let e,{lastIndex:t}=r;for(;e=r.exec(o);)yield e,{lastIndex:t}=r;if(t!=o.length)throw new Error("Failed to parse string")}const At=new Set(["area","base","basefont","bgsound","br","col","command","embed","frame","hr","image","img","input","keygen","link","meta","param","source","track","wbr"]);function Yo(r,o="  ",e=80){var u;const t=new RegExp(Zt,"gys"),a=new RegExp(Te,"gy"),c=[];let i=null,l=0,d=!1,n=0;const b=(...g)=>{for(const h of g){if(!i)if(h==`
`)d=!0;else{const $=h.indexOf(`
`),f=$==-1?h.length:$;n+f>e&&/^[ \t]+$/.test(c[c.length-1])&&(c.pop(),b(`
`)),d&&(d=!1,b(o.repeat(l)))}const m=h.lastIndexOf(`
`);m==-1?n+=h.length:n=h.length-m-1,c.push(h)}};for(const g of St(t,r)){if(Yo.__strict&&g.groups.wildcard)throw new Error("Unexpected wildcard");if(g.groups.endTag){const h=g.groups.endTagName.toLowerCase();h==i&&(i=null),i||(--l,b(`</${h}>`))}if(i)b(g[0]);else if(g.groups.space)b(...((u=g[0].match(/\n/g))==null?void 0:u.slice(0,2))??[" "]);else if(g.groups.comment||g.groups.dtd||g.groups.text||g.groups.wildcard)b(g[0]);else if(g.groups.startTag){const h=g.groups.startTagName.toLowerCase();if(b(`<${h}`),++l,g.groups.attrs){let{lastIndex:$}=a,f,q;for(;f=a.exec(g.groups.attrs);){if({lastIndex:$}=a,Yo.__strict&&f.groups.attrText)throw new Error("Unexpected attr text");f.groups.attrText?(f.groups.attrSpace&&b(/\n/.test(f.groups.attrSpace)?`
`:" "),b(f.groups.attrText)):((f.groups.attrSpace||!(q!=null&&q.groups.attrText))&&b(/\n/.test(f.groups.attrSpace)?`
`:" "),b(`${f.groups.attrName}${f.groups.quotedAttrValue?`="${f.groups.quotedAttrValue}"`:f.groups.singleQuotedAttrValue?`='${f.groups.singleQuotedAttrValue}'`:f.groups.unquotedAttrValue?`=${f.groups.unquotedAttrValue}`:""}`)),q=f}if($!=g.groups.attrs.length)throw new Error("Failed to parse attributes")}const m=!!g.groups.closingSlash;b(m?" />":">"),m||At.has(h)?--l:["pre","script","style"].includes(h)&&(i=h)}}let s=!1;for(;/^\s+$/.test(c[c.length-1]);){const g=c.pop();/\n/.test(g)&&(s=!0)}return s&&c.push(`
`),c.join("")}Yo.default=Yo;var Ct=Yo;const Et=mt(Ct);class Re extends _{static get properties(){return{source:{attribute:!1}}}firstUpdated(){let e=this.shadowRoot.querySelector("slot").assignedNodes().map(a=>a.nodeType===Node.TEXT_NODE?a.textContent:a.outerHTML).join("").trim();e=Et(e);const t=window==null?void 0:window.Prism.highlight(e,window.Prism.languages.markup,"html");this.source=t}render(){return v` <link
        rel="stylesheet"
...
.sm\\:pt-32{padding-top:3.2rem;}
  ...
  `),y(kr,"properties",{ariaLabel:{type:String,attribute:"aria-label"},clear:{type:Boolean},search:{type:Boolean},label:{type:String}});customElements.get("w-affix")||customElements.define("w-affix",kr);const Dt=()=>v`<svg
...
fill="#fff"
/>
<path d="M8.8 11.8a.8.8 0 1 1-1.6 0 .8.8 0 0 1 1.6 0Z" fill="#fff" />`,Qt=G`<circle cx="8" cy="8" r="8" transform="rotate(180 8 8)" fill="currentColor" />
<path
  fill-rule="evenodd"
  clip-rule="evenodd"
...
fill="#fff"
/>
<path d="M8.8 11.8a.8.8 0 1 1-1.6 0 .8.8 0 0 1 1.6 0Z" fill="#fff" />`,Ft=G`<circle cx="8" cy="8" r="8" fill="currentColor" />
<path
  fill-rule="evenodd"
  clip-rule="evenodd"
  d="M7.25 12a.75.75 0 0 0 1.5 0V8a.75.75 0 0 0-1.5 0v4ZM8 4a1 1 0 1 0 0 2 1 1 0 0 0 0-2Z"
  fill="#fff"
/>`;var F=function(){for(var r=[],o=arguments.length;o--;)r[o]=arguments[o];return r.reduce(function(e,t){return e.concat(typeof t=="string"?t:Array.isArray(t)?F.apply(void 0,t):typeof t=="object"&&t?Object.keys(t).map(function(a){return t[a]?a:""}):"")},[]).join(" ")};const ie={negative:Dt(),positive:Ot(),warning:It(),info:Ht()};class fr extends _{constructor(){super(),this.show=!1,this.role="alert"}connectedCallback(){if(super.connectedCallback(),!this.variant||!Object.keys(ie).includes(this.variant))throw new Error(`Invalid "variant" attribute. Set its value to one of the following:
...
.sm\\:pt-32{padding-top:3.2rem;}
      ...
      `);customElements.get("w-box")||customElements.define("w-box",qr);function xa(r,o){return r.flatMap(e=>[e,o]).slice(0,-1)}const ma=v`<span class=${go.separator} aria-hidden='true'>/</span>`;class Xr extends Ao(_){constructor(){super(),this.ariaLabel="Her er du"}connectedCallback(){super.connectedCallback();const e=Array.from(this.children).flat(1/0).filter(t=>t).map((t,a)=>{if(typeof t=="string"){const c=a===children.length-1;return v`<span class=${go.text} aria-current=${c?"page":void 0}>${t}</span>`}return t.classList.add(t.tagName==="A"?go.link:go.text),t});this._children=xa(e,ma)}render(){return v`
...
.sm\\:pt-32{padding-top:3.2rem;}
    ...
    `]);customElements.get("w-textfield")||customElements.define("w-textfield",Ar);function za(r,o){if(!wo)return;const e=customElements.get("w-toast-container").init(),t={id:Date.now().toString(36)+Math.random().toString(36).slice(2,5),text:r,duration:5e3,type:"success",...o};return e.set(t),t}function _a(r){return wo?customElements.get("w-toast-container").init().del(r):void 0}function qa(r,o){if(!wo)return;const e=customElements.get("w-toast-container").init();return e.set({...e.get(r),...o}),e.get(r)}const Hr=typeof window<"u";let rt=!0;if(Hr){const r=window.matchMedia("(prefers-reduced-motion: reduce)"),o=({matches:e})=>rt=!e;r.addEventListener&&r.addEventListener("change",o),o(r)}const et=r=>{r.style.transition=null,r.style.backfaceVisibility=null,r.style.overflow=null},tt=r=>{const o=rt?"var(--f-expansion-duration, 0.3s)":"0.01s";r.style.transition=`height ${o}`,r.style.backfaceVisibility="hidden",r.style.overflow="hidden"},Xa=(r,o)=>()=>{r.style.height="auto",r.style.overflow=null,o&&o()},Ya=r=>()=>{r&&r()},at=(r,o)=>{const e=(()=>{if(!o)return new Promise(c=>{o=c})})(),t=Xa(r,o);et(r),r.style.height="auto";let a=r.scrollHeight;if(Hr&&requestAnimationFrame(()=>{r.addEventListener("transitionend",t,{once:!0}),r.style.height="0px",r.style.transitionTimingFunction="ease-out",tt(r),requestAnimationFrame(()=>r.style.height=a+"px")}),e)return e},ct=(r,o)=>{const e=(()=>{if(!o)return new Promise(c=>{o=c})})(),t=Ya(o);et(r);let a=r.scrollHeight;if(Hr&&requestAnimationFrame(()=>{r.addEventListener("transitionend",t,{once:!0}),r.style.height=a+"px",r.style.transitionTimingFunction="ease-in",tt(r),requestAnimationFrame(()=>r.style.height="0px")}),e)return e},ja=()=>v`
  <svg
    role="img"
    aria-label="Lukk"
    xmlns="http://www.w3.org/2000/svg"
...
<path
  ...
  fill="#fff"
/>
<path d="M8.8 11.8a.8.8 0 1 1-1.6 0 .8.8 0 0 1 1.6 0Z" fill="#fff" />`,La=G`<circle cx="8" cy="8" r="8" transform="rotate(180 8 8)" fill="currentColor" />
<path
  fill-rule="evenodd"
  clip-rule="evenodd"
  d="M11.5 4.94c.3.27.34.75.06 1.06l-4 4.5a.75.75 0 0 1-1.09.03l-2-2a.75.75 0 0 1 1.06-1.06l1.44 1.44L10.44 5a.75.75 0 0 1 1.06-.07Z"
...
.sm\\:pt-32{padding-top:3.2rem;}
    ...
    `]),y(Ko,"properties",{_toasts:{state:!0}});customElements.get("w-toast-container")||customElements.define("w-toast-container",Ko);const it=Z`
...
`}render(){return this.level?Dr(this._markup):v`<slot></slot>`}}y(Lr,"properties",{level:{type:Number}}),y(Lr,"styles",[it,Z`
    /* layer: preflights */
*,::before,::after{--w-rotate:0;--w-rotate-x:0;--w-rotate-y:0;--w-rotate-z:0;--w-scale-x:1;--w-scale-y:1;--w-scale-z:1;--w-skew-x:0;--w-skew-y:0;--w-translate-x:0;--w-translate-y:0;--w-translate-z:0;}
*,:before,:after{box-sizing:border-box;border-style:solid;border-width:0;border-color:var(--w-s-color-border,var(--w-s-color-border-default))}html{font-size:62.5%}body{background-color:var(--w-s-color-background,var(--w-s-color-background-default));min-height:100%;margin:0;overflow-y:scroll}body,:host{-webkit-text-size-adjust:100%;tab-size:4;-webkit-tap-highlight-color:transparent;font-family:var(--w-font-family);font-size:var(--w-font-size-m);line-height:var(--w-line-height-m);color:var(--w-s-color-text,var(--w-s-color-text-default))}hr{height:0;color:inherit;border-top-width:1px}abbr:where([title]){-webkit-text-decoration:underline dotted;text-decoration:underline dotted}h1,h2,h3,h4,h5,h6{font-size:inherit;font-weight:700}a{cursor:pointer;color:var(--w-s-color-text-link);text-decoration:none}a:hover,a:focus,a:active{-webkit-text-decoration:var(--w-decoration-text-link);text-decoration:var(--w-decoration-text-link)}b,strong{font-weight:700}code,kbd,samp,pre{font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,Liberation Mono,Courier New,monospace;font-size:1em}sub,sup{vertical-align:baseline;font-size:75%;line-height:0;position:relative}sub{bottom:-.25em}sup{top:-.5em}table{text-indent:0;border-color:inherit;border-collapse:collapse}button,input,optgroup,select,textarea{font-family:inherit;font-size:100%;font-weight:inherit;line-height:inherit;color:inherit;margin:0;padding:0}button,select{text-transform:none}button,[type=button],[type=reset],[type=submit]{-webkit-appearance:button}:-moz-focusring{outline:auto}:-moz-ui-invalid{box-shadow:none}progress{vertical-align:baseline}::-webkit-inner-spin-button,::-webkit-outer-spin-button{height:auto}[type=search]{-webkit-appearance:textfield;outline-offset:-2px}::-webkit-search-decoration{-webkit-appearance:none}::-webkit-file-upload-button{-webkit-appearance:button;font:inherit}summary{display:list-item}blockquote,dl,dd,h1,h2,h3,h4,h5,h6,hr,figure,p,pre{margin:0}fieldset{margin:0;padding:0}legend{padding:0}ol,ul,menu{margin:0;padding:0;list-style:none}textarea{resize:vertical}input::placeholder,textarea::placeholder{opacity:1;color:var(--w-s-color-text-placeholder)}button,[role=button]{cursor:pointer}:disabled{cursor:default}img,svg,video,canvas,audio,iframe,embed,object{vertical-align:middle;display:block}img,video{max-width:100%;height:auto}h1{font-size:var(--w-font-size-xxl);line-height:var(--w-line-height-xxl)}h2{font-size:var(--w-font-size-xl);line-height:var(--w-line-height-xl)}h3{font-size:var(--w-font-size-l);line-height:var(--w-line-height-l)}h4{font-size:var(--w-font-size-m);line-height:var(--w-line-height-m)}h5{font-size:var(--w-font-size-s);line-height:var(--w-line-height-s)}dt,dd{margin:0 16px}h1,h2,h3,h4,h5,ul,ol,dl,p,blockquote{margin:0 0 8px}[hidden]{display:none!important}[tabindex="-1"]:focus:not(:focus-visible){outline:none}legend{float:left;width:100%;margin:0;padding:0;display:table}legend+*{clear:both}fieldset{min-width:0;border:0;margin:0;padding:.01em 0 0}body:not(:-moz-handler-blocked) fieldset{display:table-cell}
/* layer: shortcuts */
...
.sm\\:pt-32{padding-top:3.2rem;}
  ...
  `);customElements.define("sidebar-element",lt);
