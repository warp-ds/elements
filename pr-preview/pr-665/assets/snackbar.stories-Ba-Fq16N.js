...
`})),S,C,w=e((()=>{i(),d(),h(),x(),l(),S=function(e){return e[e.Short=5e3]=`Short`,e[e.Long=2e4]=`Long`,e[e.Infinite=12e7]=`Infinite`,e}({}),C=class extends n{constructor(...e){super(...e),this.actionPlacement=`inline`,this.duration=S.Short,this.#e=S.Short,this.#t=null,this.#n=null}static{this.styles=[m,b]}#e;#t;#n;connectedCallback(){if(super.connectedCallback(),this.duration!==S.Infinite){let e=1e3;this.#e=this.duration-e,this.#n=setInterval(()=>{this.#t===null&&(this.#e-=e,this.#e<=0&&this.close())},e)}}disconnectedCallback(){this.#n&&clearInterval(this.#n)}close(){this.remove(),this.#t&&this.#t.focus()}#r(e){this.#t||e.relatedTarget&&e.relatedTarget.focus&&(this.#t=e.relatedTarget)}#i(e){(!e.relatedTarget||!this.shadowRoot?.host.contains(e.relatedTarget))&&(this.#t=null)}render(){return r`
            <div part="item" @focusin=${this.#r} @focusout=${this.#i}>
                <div part="icon">
                    <slot name="icon"></slot>
                </div>
                <div part="message">
...
`})),L,R,z,B=e((()=>{a(),i(),w(),s(),E(),O(),A(),M(),P(),h(),I(),L={canClose:!0,duration:S.Short,variant:`neutral`},c(D,j,k,T,N),R=/\s/,z=class extends n{static{this.styles=[m,F]}constructor(){super(),this.internals=this.attachInternals(),this.internals.ariaLive=`polite`,this.internals.role=`log`,this._onKeydown=this._onKeydown.bind(this)}connectedCallback(){super.connectedCallback(),document.addEventListener(`keydown`,this._onKeydown)}disconnectedCallback(){super.disconnectedCallback(),document.removeEventListener(`keydown`,this._onKeydown)}_onKeydown(e){if(e.key===`Escape`){let e=this.shadowRoot?.host.querySelector(`w-snackbar-item:last-of-type`);e&&e.close()}}create(e,t={}){let n={...L,...t},r=document.createElement(`w-snackbar-item`);if(n.duration&&(r.duration=n.duration),r.innerText=e,n.variant!==`neutral`){let e=document.createElement(`w-icon`);switch(e.slot=`icon`,n.variant){case`positive`:e.name=`SuccessFilled`;break;case`warning`:e.name=`WarningFilled`;break;case`negative`:e.name=`ErrorFilled`;break;case`info`:e.name=`InfoFilled`;break}r.prepend(e)}if(n.action){r.duration<S.Long&&(r.duration=S.Long);let e=document.createElement(`w-button`);e.slot=`action`,e.variant=`utilityQuiet`,e.small=!0;let{label:t,onclick:i,placement:a}=n.action;e.innerText=t,e.onclick=i,a?r.actionPlacement=a:R.test(t)&&(r.actionPlacement=`block`),r.append(e)}if(r.duration>=S.Long&&(n.canClose=!0),n.canClose){let e=document.createElement(`w-button`);e.slot=`action`,e.variant=`utilityQuiet`,e.small=!0,e.iconOnly=!0,typeof n.canClose==`function`?e.onclick=t=>{n.canClose.call(e,t),!t.defaultPrevented&&r.close.call(r)}:e.onclick=r.close.bind(r);let t=document.createElement(`w-icon`);t.name=`Close`,t.size=`small`,t.ariaLabel=o.t({id:`snackbar.aria.close`,message:`Dismiss message`,comment:`Accessibility label for the button that closes the snackbar/toast popup`}),e.append(t),r.append(e)}return this.shadowRoot?.host.prepend(r),r}render(){return r`
