import{n as e,r as t}from"./rolldown-runtime-DaJ6WEGw.js";import{c as n,d as r,h as i,l as a,s as o,t as s,x as c,y as l}from"./decorators-BoKEMuNh.js";import{a as u,o as d,r as f,t as p}from"./i18n-Cu-tp5qV.js";import{n as m,t as h}from"./dist-B-Y-CnmQ.js";import{n as g,t as _}from"./styles-CVeKeHRg.js";import{a as v,n as y,r as b,t as x}from"./utilities-DIER-odo.js";function S(e,t){return e.flatMap(e=>[e,t]).slice(0,-1)}var C=e((()=>{})),w,T=e((()=>{w=JSON.parse(`{"breadcrumbs.ariaLabel":["Du er her"]}`)})),E,D=e((()=>{E=JSON.parse(`{"breadcrumbs.ariaLabel":["You are here"]}`)})),O,k=e((()=>{O=JSON.parse(`{"breadcrumbs.ariaLabel":["Olet tässä"]}`)})),A,j=e((()=>{A=JSON.parse(`{"breadcrumbs.ariaLabel":["Her er du"]}`)})),M,N=e((()=>{M=JSON.parse(`{"breadcrumbs.ariaLabel":["Du är här"]}`)})),P,F=e((()=>{n(),P=c(`*,:before,:after{--w-rotate:0;--w-rotate-x:0;--w-rotate-y:0;--w-rotate-z:0;--w-scale-x:1;--w-scale-y:1;--w-scale-z:1;--w-skew-x:0;--w-skew-y:0;--w-translate-x:0;--w-translate-y:0;--w-translate-z:0}.flex{display:flex}.static{position:static}.s-text{color:var(--w-s-color-text)}.s-text-link{color:var(--w-s-color-text-link)}.s-icon{color:var(--w-s-color-icon)}.space-x-8>:not([hidden])~:not([hidden]){--w-space-x-reverse:0;margin-left:calc(.8rem * calc(1 - var(--w-space-x-reverse)));margin-right:calc(.8rem * var(--w-space-x-reverse))}.sr-only{clip:rect(0,0,0,0);white-space:nowrap;border-width:0;width:1px;height:1px;margin:-1px;padding:0;position:absolute;overflow:hidden}.select-none{-webkit-user-select:none;user-select:none}`)})),I,L,R,z=e((()=>{d(),C(),n(),f(),_(),T(),D(),k(),j(),N(),F(),I={wrapper:`flex space-x-8`,text:`s-text`,link:`s-text-link`,separator:`select-none s-icon`,a11y:`sr-only`},L=i`<span class=${I.separator}>/</span>`,R=class extends a{static{this.styles=[g,P]}constructor(){super(),this._children=[],this._internals=this.attachInternals(),p(E,A,O,w,M),this._defaultLabel=u._({id:`breadcrumbs.ariaLabel`,message:`You are here`,comment:`Default screen reader message for the breadcrumb component`}),this._internals.ariaLabel=this._defaultLabel}get _label(){return this.ariaLabel??this._defaultLabel}connectedCallback(){super.connectedCallback(),this._internals.role=`navigation`;let e=this.querySelectorAll(`:scope > a`),t=this.querySelectorAll(`:scope > span`),n=this.querySelectorAll(`w-breadcrumb-item`);n.length>0&&(e.length>0||t.length>0)&&console.warn(`Mixing Legacy API and w-breadcrumb-item API children is not supported.`);let r=0,a=!1;n.forEach((e,t)=>{e.hasAttribute(`current-page`)&&(t<n.length-1&&(a=!0),r++)}),r>1&&console.warn(`Please ensure only one w-breadcrumb-item has the current-page attribute.`),a&&console.warn(`The current-page attribute should only be used on the final breadcrumb item.`);let o=[...this.children].filter(e=>e&&e.tagName!==`W-BREADCRUMB-ITEM`).flat(1/0).filter(e=>e).map((e,t)=>{if(typeof e==`string`){let n=t===this.children.length-1;return i`<span
					class=${I.text}
					aria-current=${n?`page`:void 0}
					>${e}</span
				>`}return e.classList.add(e.tagName===`A`?I.link:I.text),e});this._children=S(o,L)}render(){return i`
			<nav aria-labelledby="breadCrumbLabel">
				<h2 id="breadCrumbLabel" class=${I.a11y}>${this._label}</h2>
				<div class=${I.wrapper}>${this._children}<slot></slot></div>
			</nav>
		`}},customElements.get(`w-breadcrumbs`)||customElements.define(`w-breadcrumbs`,R)})),B,V=e((()=>{n(),B=l`
	:host {
		display: inline-block;

		/* Internal resolved vars (defaults) */
		--_link-color: var(
			--w-c-breadcrumb-item-link-color,
			var(--w-s-color-text-link)
		);
		--_text-color: var(--w-c-breadcrumb-item-text-color, var(--w-s-color-text));
		--_separator-color: var(
			--w-c-breadcrumb-item-separator-color,
			var(--w-s-color-icon)
		);
		--_separator-spacing: var(--w-c-breadcrumb-item-separator-spacing, 0.8rem);
		--_font-size: var(--w-c-breadcrumb-item-font-size);
		--_line-height: var(--w-c-breadcrumb-item-line-height, 1.5);
		--_font-weight: var(--w-c-breadcrumb-item-font-weight, 400);
		--_padding-x: var(--w-c-breadcrumb-item-padding-x, 0);
		--_padding-y: var(--w-c-breadcrumb-item-padding-y, 0);
		--_link-color-hover: var(
			--w-c-breadcrumb-item-link-color-hover,
			var(--w-s-color-text-link)
		);
		--_link-color-active: var(
			--w-c-breadcrumb-item-link-color-active,
			var(--w-s-color-text-link)
		);
		--_outline-color: var(
			--w-c-breadcrumb-item-outline-color,
			var(--w-s-color-border-focus)
		);
		--_outline-width: var(--w-c-breadcrumb-item-outline-width, 2px);
		--_outline-offset: var(--w-c-breadcrumb-item-outline-offset, 1px);
	}

	.s-text {
		color: var(--_text-color);
		font-size: var(--_font-size);
		line-height: var(--_line-height);
		font-weight: var(--_font-weight);
	}
	.s-text-link {
		color: var(--_link-color);
		text-decoration: none;
		font-size: var(--_font-size);
		line-height: var(--_line-height);
		font-weight: var(--_font-weight);
		padding-left: var(--_padding-x);
		padding-right: var(--_padding-x);
		padding-top: var(--_padding-y);
		padding-bottom: var(--_padding-y);
	}
	.s-text-link:hover {
		text-decoration: underline;
		color: var(--_link-color-hover);
	}
	.s-text-link:active {
		color: var(--_link-color-active);
	}
	.s-text-link:focus {
		outline-color: var(--_outline-color);
		outline-width: var(--_outline-width);
		outline-offset: var(--_outline-offset);
	}
	.separator {
		color: var(--_separator-color);
		-webkit-user-select: none;
		user-select: none;
		font-size: var(--_font-size);
		line-height: var(--_line-height);
		font-weight: var(--_font-weight);
		margin-left: var(--_separator-spacing);
		margin-right: var(--_separator-spacing);
	}
`})),H,U=e((()=>{n(),s(),V(),_(),H=class extends a{@o({type:Boolean,attribute:`current-page`})currentPage=!1;@o({type:String})href=null;static styles=[g,B];get link(){return this.href?i`<a
					part="link"
					class="s-text-link"
					href=${this.href}
					aria-current=${this.currentPage?`page`:r}
					><slot></slot
				></a>`:i`<span
					part="text"
					class="s-text"
					aria-current=${this.currentPage?`page`:r}
					><slot></slot
				></span>`}get separator(){return this.currentPage?r:i`<span part="separator" class="separator" aria-hidden="true"
					>/</span
				>`}render(){return i`${this.link}${this.separator}`}},customElements.get(`w-breadcrumb-item`)||customElements.define(`w-breadcrumb-item`,H)})),W=t({Default:()=>J,WithLinks:()=>Y,WithSpanForCurrentPage:()=>X,__namedExportsOrder:()=>Z,default:()=>q}),G,K,q,J,Y,X,Z,Q=e((()=>{b(),m(),n(),x(),z(),U(),{events:G,argTypes:K}=h(`w-breadcrumbs`),q={title:`Navigation/Breadcrumbs`,render(e){return i`
            <w-breadcrumbs ${v(y(e))}>
                <w-breadcrumb-item href="#/home">Home</w-breadcrumb-item>
                <w-breadcrumb-item href="#/category">Category</w-breadcrumb-item>
                <w-breadcrumb-item current-page>Current page</w-breadcrumb-item>
            </w-breadcrumbs>
        `},args:{"aria-label":`You are here`},argTypes:K,parameters:{actions:{handles:G}}},J={},Y={render(e){return i`
            <w-breadcrumbs ${v(y(e))}>
                <w-breadcrumb-item href="#/home">Home</w-breadcrumb-item>
                <w-breadcrumb-item href="#/category">Category</w-breadcrumb-item>
                <w-breadcrumb-item href="#" current-page
                    >Current page</w-breadcrumb-item
                >
            </w-breadcrumbs>
        `}},X={render(e){return i`
            <w-breadcrumbs ${v(y(e))}>
                <w-breadcrumb-item href="#/home">Home</w-breadcrumb-item>
                <w-breadcrumb-item href="#/category">Category</w-breadcrumb-item>
                <w-breadcrumb-item current-page>Current page</w-breadcrumb-item>
            </w-breadcrumbs>
        `}},J.parameters={...J.parameters,docs:{...J.parameters?.docs,source:{originalSource:`{}`,...J.parameters?.docs?.source}}},Y.parameters={...Y.parameters,docs:{...Y.parameters?.docs,source:{originalSource:`{
  render(args) {
    return html\`
            <w-breadcrumbs \${spread(prespread(args))}>
                <w-breadcrumb-item href="#/home">Home</w-breadcrumb-item>
                <w-breadcrumb-item href="#/category">Category</w-breadcrumb-item>
                <w-breadcrumb-item href="#" current-page
                    >Current page</w-breadcrumb-item
                >
            </w-breadcrumbs>
        \`;
  }
}`,...Y.parameters?.docs?.source}}},X.parameters={...X.parameters,docs:{...X.parameters?.docs,source:{originalSource:`{
  render(args) {
    return html\`
            <w-breadcrumbs \${spread(prespread(args))}>
                <w-breadcrumb-item href="#/home">Home</w-breadcrumb-item>
                <w-breadcrumb-item href="#/category">Category</w-breadcrumb-item>
                <w-breadcrumb-item current-page>Current page</w-breadcrumb-item>
            </w-breadcrumbs>
        \`;
  }
}`,...X.parameters?.docs?.source}}},Z=[`Default`,`WithLinks`,`WithSpanForCurrentPage`]}));Q();export{J as Default,Y as WithLinks,X as WithSpanForCurrentPage,Z as __namedExportsOrder,q as default,Q as n,W as t};