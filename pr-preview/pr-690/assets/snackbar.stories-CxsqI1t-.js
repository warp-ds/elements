...
`})),H,U,W,G=e((()=>{a(),i(),k(),b(),j(),N(),F(),L(),z(),h(),V(),H={canClose:!0,duration:D.Short,variant:`neutral`},d(M,I,P,A,R),U=/\s/,W=class extends r{static{this.styles=[v,B]}constructor(){super(),this.internals=this.attachInternals(),this.internals.ariaLive=`polite`,this.internals.role=`log`,this._onKeydown=this._onKeydown.bind(this)}connectedCallback(){super.connectedCallback(),document.addEventListener(`keydown`,this._onKeydown)}disconnectedCallback(){super.disconnectedCallback(),document.removeEventListener(`keydown`,this._onKeydown)}_onKeydown(e){if(e.key===`Escape`){let e=this.shadowRoot?.host.querySelector(`w-snackbar-item:last-of-type`);e&&e.close()}else if(e.altKey&&e.key===`Enter`){e.preventDefault();let t=this.shadowRoot?.host.querySelector(`w-snackbar-item:last-of-type`);if(console.log(`Found item!`),t){let e=t.querySelector(`w-button[slot="action"]`);console.log(`Found action!`),e&&(e.click(),console.log(`Clicked!`))}}}create(e,t={}){let n={...H,...t},r=document.createElement(`w-snackbar-item`);if(n.duration&&(r.duration=n.duration),r.innerText=e,n.variant!==`neutral`){let e=document.createElement(`w-icon`);switch(e.slot=`icon`,n.variant){case`positive`:e.name=`SuccessFilled`;break;case`warning`:e.name=`WarningFilled`;break;case`negative`:e.name=`ErrorFilled`;break;case`info`:e.name=`InfoFilled`;break}r.prepend(e)}if(n.action){r.duration<D.Long&&(r.duration=D.Long);let e=document.createElement(`w-button`);e.slot=`action`,e.variant=`utilityQuiet`,e.small=!0;let{label:t,onclick:i,placement:a}=n.action;e.innerText=t;let o=document.createElement(`span`);o.classList.add(`sr-only`),o.innerHTML=`<kbd>${window.navigator.userAgent.includes(`Macintosh`)?`Option`:`Alt`}</kbd> + <kbd>Enter</kbd>`,e.append(o),e.onclick=t=>{let n=i.call(e,t);return t.defaultPrevented||r.close(),n},a?r.actionPlacement=a:U.test(t)&&(r.actionPlacement=`block`),r.append(e)}if(r.duration>D.Long&&(n.canClose=!0),n.canClose){let e=document.createElement(`w-button`);e.slot=`close`,e.variant=`utilityQuiet`,e.small=!0,e.iconOnly=!0,typeof n.canClose==`function`?e.onclick=t=>{let i=n.canClose.call(e,t);return t.defaultPrevented||r.close(),i}:e.onclick=r.close.bind(r);let t=document.createElement(`w-icon`);t.name=`Close`,t.size=`small`,t.ariaLabel=o.t({id:`snackbar.aria.close`,message:`Dismiss message`,comment:`Accessibility label for the button that closes the snackbar/toast popup`}),e.append(t),r.append(e)}return this.shadowRoot?.host.prepend(r),r}render(){return t`
		...
		`}},customElements.get(`w-snackbar`)||customElements.define(`w-snackbar`,W)})),K,q,J,Y,X,Z,Q,$;e((()=>{y(),i(),ne(),x(),S(),m(),w(),C(),T(),G(),k(),{events:K,args:q,argTypes:J}=f(`w-snackbar`),Y={title:`Overlays/Snackbar`,args:{...q,canClose:!0,duration:void 0},argTypes:{...J,duration:{control:{type:`select`},options:[`4 seconds`,`10 seconds`,`Infinite`]}},parameters:{actions:{handles:K}}},X={render({duration:e,canClose:n}){let r=e===`Infinite`?D.Infinite:e===`10 seconds`?D.Long:D.Short;return t`
            <div class="flex gap-8">
                <w-button
                    @click="${()=>{document.querySelector(`w-snackbar`).create(`The message should be short, but if it is not and spans multiple lines then the icon and actions should be centered vertically`,{variant:`positive`,duration:r,canClose:n})}}"
                    >Positive variant</w-button
                >

                <w-button
                    @click="${()=>{document.querySelector(`w-snackbar`).create(`The message should be short`,{variant:`warning`,duration:r,canClose:n})}}"
                    >Warning variant</w-button
                >

                <w-button
                    @click="${()=>{document.querySelector(`w-snackbar`).create(`The message should be short`,{variant:`negative`,duration:r,canClose:n})}}"
                    >Negative variant</w-button
                >

                <w-button
                    @click="${()=>{document.querySelector(`w-snackbar`).create(`The message should be short`,{variant:`info`,duration:r,canClose:n})}}"
                    >Info variant</w-button
                >

                <w-button
                    @click="${()=>{document.querySelector(`w-snackbar`).create(`The message should be short`,{duration:r,canClose:n})}}"
                    >Neutral variant</w-button
                >

                <w-button
                    @click="${()=>{let e=document.querySelector(`w-snackbar`).create(`The message should be short`,{duration:r,canClose:n}),t=document.createElement(`w-icon`);t.name=`Rocket`,t.slot=`icon`,e.prepend(t)}}"
        ...
        `}},Z={render({duration:e,canClose:n}){let r=e===`Infinite`?D.Infinite:e===`10 seconds`?D.Long:D.Short;return t`
            <div class="flex gap-8">
                <w-button
                    @click="${()=>{document.querySelector(`w-snackbar`).create(`The message should be short, but if it is not and spans multiple lines then the icon and actions should be centered vertically`,{variant:`positive`,duration:r,canClose:n,action:{label:`Action`,onclick(){let e=document.getElementById(`with-action-modal`);e.show=!0}}})}}"
                    >Positive variant</w-button
                >

                <w-button
                    @click="${()=>{document.querySelector(`w-snackbar`).create(`The message should be short`,{variant:`warning`,duration:r,canClose:n,action:{label:`Action`,onclick(){let e=document.getElementById(`with-action-modal`);e.show=!0}}})}}"
                    >Warning variant</w-button
                >

                <w-button
                    @click="${()=>{document.querySelector(`w-snackbar`).create(`The message should be short`,{variant:`negative`,duration:r,canClose:n,action:{label:`Action`,onclick(){let e=document.getElementById(`with-action-modal`);e.show=!0}}})}}"
                    >Negative variant</w-button
                >

                <w-button
                    @click="${()=>{document.querySelector(`w-snackbar`).create(`The message should be short`,{variant:`info`,duration:r,canClose:n,action:{label:`Action`,onclick(){let e=document.getElementById(`with-action-modal`);e.show=!0}}})}}"
                    >Info variant</w-button
                >

                <w-button
                    @click="${()=>{document.querySelector(`w-snackbar`).create(`The message should be short`,{duration:r,canClose:n,action:{label:`Action`,onclick(){let e=document.getElementById(`with-action-modal`);e.show=!0}}})}}"
                    >Neutral variant</w-button
                >
            </div>
        ...
        `}},Q={render({duration:e,canClose:n}){let r=e===`Infinite`?D.Infinite:e===`10 seconds`?D.Long:D.Short;return t`
            <div class="flex gap-8">
                <w-button
                    @click="${()=>{document.querySelector(`w-snackbar`).create(`The message should be short, but if it is not and spans multiple lines then the icon and actions should be centered vertically`,{variant:`positive`,duration:r,canClose:n,action:{label:`Long action label`,onclick(){let e=document.getElementById(`with-long-action-modal`);e.show=!0}}})}}"
                    >Positive variant</w-button
                >

                <w-button
                    @click="${()=>{document.querySelector(`w-snackbar`).create(`The message should be short`,{variant:`warning`,duration:r,canClose:n,action:{label:`Long action label`,onclick(){let e=document.getElementById(`with-long-action-modal`);e.show=!0}}})}}"
                    >Warning variant</w-button
                >

                <w-button
                    @click="${()=>{document.querySelector(`w-snackbar`).create(`The message should be short`,{variant:`negative`,duration:r,canClose:n,action:{label:`Long action label`,onclick(){let e=document.getElementById(`with-long-action-modal`);e.show=!0}}})}}"
                    >Negative variant</w-button
                >

                <w-button
                    @click="${()=>{document.querySelector(`w-snackbar`).create(`The message should be short`,{variant:`info`,duration:r,canClose:n,action:{label:`Long action label`,onclick(){let e=document.getElementById(`with-long-action-modal`);e.show=!0}}})}}"
                    >Info variant</w-button
                >

                <w-button
                    @click="${()=>{document.querySelector(`w-snackbar`).create(`The message should be short`,{duration:r,canClose:n,action:{label:`Long action label`,onclick(){let e=document.getElementById(`with-long-action-modal`);e.show=!0}}})}}"
                    >Neutral variant</w-button
                >
            </div>
