import{i as e}from"./preload-helper-xPQekRTU.js";import{_ as t,d as n,i as r,l as i,n as a,p as o,r as s,s as ee,t as c,u as l,x as u}from"./decorate-BxgRj7DM.js";import{M as te,_ as ne,d as re,j as d,l as f,u as p,v as m,y as h}from"./iframe-BKE-0EoA.js";import{r as ie,t as ae}from"./if-defined-B1ZLCare.js";function g(e){let t=-1;e.ontouchstart=e=>{e.targetTouches.length===1&&(t=e.targetTouches[0].clientY)},e.ontouchmove=n=>{if(n.targetTouches.length!==1)return;let r=n.targetTouches[0].clientY-t;return e.scrollTop===0&&r>0||E(e)&&r<0?x(n):(n.stopPropagation(),!0)}}function oe(e){e.ontouchstart=null,e.ontouchmove=null}function se(e){if(!e)throw Error(`Could not run setup, an element must be provided`);v.some(t=>t===e)||(v.length||(C(),O()),g(e),v.push(e))}function _(){v.forEach(oe),k(),T(),v=[]}var v,y,b,x,S,C,w,T,E,D,O,k,A=e((()=>{v=[],y={documentElement:{},body:{}},b=Object.freeze({documentElement:{"scrollbar-gutter":`stable`},body:{overflow:`hidden`,position:`relative`,height:`100%`}}),x=e=>{e.touches.length>1||e.preventDefault?.()},S=e=>([t,n])=>{y[e][t]=document[e].style[t],document[e].style[t]=n},C=()=>Object.entries(b).forEach(([e,t])=>{Object.entries(t).forEach(S(e))}),w=e=>([t,n])=>{document[e].style[t]=y[e][t]},T=()=>Object.entries(y).forEach(([e,t])=>{Object.entries(t).forEach(w(e))}),E=e=>e.scrollHeight-Math.abs(e.scrollTop)===e.clientHeight,D=e=>()=>document[e?`addEventListener`:`removeEventListener`](`touchmove`,x,{passive:!1}),O=D(!0),k=D()})),j,ce=e((()=>{l(),j=u`
	:host {
		--_backdrop-bg: var(
...
`})),Z,he=e((()=>{W(),te(),l(),s(),re(),h(),le(),ue(),de(),fe(),pe(),P(),f(),me(),a(),Z=class extends M(n){static{this.styles=[p,X]}constructor(){super(),this.back=!1,this.noClose=!1,this._hasTopContent=!1,ne(K,J,q,G,Y)}async willUpdate(e){if(e.has(`back`)){if(!this.titleEl)return;new U(this.titleEl).when(async()=>{await this.updateComplete})}}emitBack(){this.dispatchEvent(new CustomEvent(`backClicked`,{bubbles:!0,composed:!0}))}handleTopSlotChange(e){let t=e.target.assignedElements({flatten:!0});this._hasTopContent=!!t.length}render(){let e=this.back&&!this._hasTopContent,n=!this.noClose;return t`
			<div
				part="header"
			...
			>
				...
				${e?t`<w-button
							...
							part="back"
							title=""
							aria-label="${d._({id:`modal.aria.back`,message:`Back`,comment:`Aria label for the back button in modal`})}"
							variant="pill"
							icon-only
							@click="${this.emitBack}"
						>
				...
				${n?t`<w-button
							...
							part="close"
							title=""
							aria-label="${d._({id:`modal.aria.close`,message:`Close`,comment:`Aria label for the close button in modal`})}"
							variant="pill"
							icon-only
							@click="${this.close}"
						>
		...
		`}},c([i({type:String})],Z.prototype,`title`,void 0),c([i({type:Boolean})],Z.prototype,`back`,void 0),c([i({type:Boolean,attribute:`no-close`})],Z.prototype,`noClose`,void 0),c([ee()],Z.prototype,`_hasTopContent`,void 0),c([r(`[part="title"]`)],Z.prototype,`titleEl`,void 0),customElements.get(`w-modal-header`)||customElements.define(`w-modal-header`,Z)})),Q,ge=e((()=>{l(),Q=u`
	:host {
...
`})),$,_e=e((()=>{l(),P(),f(),ge(),$=class extends M(N(n)){static{this.styles=[p,Q]}render(){return t`
			<slot part="footer" @slotchange="${this.handleSlotChange}"></slot>
		`}},customElements.get(`w-modal-footer`)||customElements.define(`w-modal-footer`,$)}));export{he as n,I as r,_e as t};