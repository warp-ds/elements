      ...
      `,await t({roots:[this.shadowRoot],elements:[this.anchorPositioningStyleElement]})}catch(e){console.error(Error(`Error registering the CSS anchor positioning polyfill. The UI will look broken.`,{cause:e}))}}let e=window.navigator.userAgent;O.test(e)&&!k.test(e)&&this.tooltipTarget&&this.tooltipTarget.style.setProperty(`--transform-offset`,`var(--w-slider-thumb-size, 28px)`),this.#r()}get boundaryValue(){return this.slot===`from`?this.min:this.max}get textFieldDisplayValue(){return this._inputHasFocus&&this.value===``?this.range?.value?Math.min(Math.max(Number(this.range.value),Number(this.min)+1),Number(this.max)-1).toString():``:this.value}get tooltipDisplayValue(){let e;return e=this.tooltipFormatter?this.tooltipFormatter(this.value,this.slot):this.valueFormatter?this.valueFormatter(this.value,this.slot):this.value===``?this.range?.value??this.boundaryValue:this.value||0,e}get ariaDescriptionText(){let e=``,t=this.ariaDescription||``,n=this.value===``;return this.openEnded&&n&&(e=this.slot===`from`?_.t({id:`slider.placeholder.from`,message:`Min`}):_.t({id:`slider.placeholder.to`,message:`Max`})),e?`${e}, ${t}`:t}updated(e){if(e.has(`openEnded`)&&this.openEnded&&!this.placeholder&&(this.slot===`to`||this.slot===``?this.placeholder=_.t({id:`slider.placeholder.to`,message:`Max`,comment:`Max as in short for Maximum`}):this.slot===`from`&&(this.placeholder=_.t({id:`slider.placeholder.from`,message:`Min`,comment:`Min as in short for Minimum`}))),e.has(`value`)){this.#e===void 0&&this.value!==void 0&&(this.#e=this.value),this.setValue(this.value),this.#r();let t=e.get(`value`);this.value===``&&t&&this.dispatchEvent(new CustomEvent(`thumbreset`,{bubbles:!0}))}}render(){let e=this.placeholder&&!this.value,t=this.tooltipDisplayValue!==`Min`&&this.tooltipDisplayValue!==`Max`&&(this.suffix??``)!==``;return o`
			<div class="w-slider-thumb">
				...
				>
					...
					></output>
					<span slot="message">
						${this.tooltipDisplayValue}${t?o`&nbsp;${this.suffix??``}`:i}
					</span>
				</w-attention>
