    ...
    `]}constructor(){super(),this.pages=0,this.currentPageNumber=1,this.visiblePages=7,d(D,j,k,T,N)}get shouldShowShowFirstPageButton(){return this.currentPageNumber-2>0}get shouldShowPreviousPageButton(){return this.currentPageNumber-1>0}get shouldShowNextPageButton(){return this.currentPageNumber<this.pages}get currentPageIndex(){return this.currentPageNumber-1}get visiblePageNumbers(){if(this.pages<=this.visiblePages)return Array.from({length:this.pages},(e,t)=>t+1);let e=Math.floor(this.visiblePages/2),t=Math.max(1,this.currentPageNumber-e),n=Math.min(this.pages,t+this.visiblePages-1);return n-t+1<this.visiblePages&&(t=Math.max(1,n-this.visiblePages+1)),Array.from({length:n-t+1},(e,n)=>t+n)}#e(e){let t=e.target.getAttribute(`data-page-number`);this.dispatchEvent(new CustomEvent(`page-click`,{detail:{clickedPage:t},bubbles:!0,composed:!0}))}render(){let e=this.visiblePageNumbers;return a`<nav class="flex items-center justify-center p-8" @click="${this.#e}">
      ...
      <div class="flex items-center s-text-link">
        ${this.shouldShowShowFirstPageButton?a`<a
              ...
              >
              <w-icon name="ChevronDoubleLeft" size="small" locale="${l()}" class="pointer-events-none flex items-center" class="flex"></w-icon>
        ...
        ${this.shouldShowPreviousPageButton?a`<a
              ...
              >
              <w-icon name="ChevronLeft" size="small" locale="${l()}" class="pointer-events-none flex items-center" class="flex"></w-icon>
              <span class="sr-only">${L()}</span>
            </a>`:i}
        <div class="hidden md:block">
          ${e.map(e=>{let t=e===this.currentPageNumber,n=`${this.baseUrl}${e}`,r=R;return t?r+=` s-bg-primary s-text-inverted`:r+=` hover:bg-[--w-color-button-pill-background-hover] active:bg-[--w-color-button-pill-background-active]`,a`<a
              data-page-number="${e}"
              aria-label="${c._({id:`pagination.aria.page`,message:`Page {currentPage}`,values:{currentPage:e},comment:`Default screenreader message for page link in the pagination component`})}"
              href="${n}"
              class="${r}"
              aria-current="${t?`page`:``}"
              >${e}</a
        ...
        ${this.shouldShowNextPageButton?a`<a
              ...
              >
              <w-icon name="ChevronRight" size="small" locale="${l()}" class="pointer-events-none flex items-center" class="flex"></w-icon>
              <span class="sr-only">${L()}</span>
            </a>`:i}
      </div>
    </nav>`}},h([p({type:String,reflect:!0,attribute:`base-url`})],z.prototype,`baseUrl`,void 0),h([p({type:Number,reflect:!0,useDefault:!0})],z.prototype,`pages`,void 0),h([p({type:Number,reflect:!0,attribute:`current-page`,useDefault:!0})],z.prototype,`currentPageNumber`,void 0),h([p({type:Number,reflect:!0,attribute:`visible-pages`,useDefault:!0})],z.prototype,`visiblePages`,void 0),customElements.get(`w-pagination`)||customElements.define(`w-pagination`,z)})),V=t({BasicPagination:()=>K,LastPageSelected:()=>J,ManyPagesWithLimitedVisible:()=>Y,MiddlePageSelected:()=>q,SinglePage:()=>X,__namedExportsOrder:()=>Z,default:()=>G}),H,U,W,G,K,q,J,Y,X,Z,Q=e((()=>{C(),b(),o(),w(),B(),{events:H,args:U,argTypes:W}=y(`w-pagination`),G={title:`Navigation/Pagination`,render(e){return a`<w-pagination ${x(S(e))}></w-pagination>`},args:U,argTypes:W,parameters:{actions:{handles:H}}},K={args:{"current-page":1,pages:5,"base-url":`/search?page=`}},q={args:{"current-page":3,pages:7,"base-url":`/search?page=`}},J={args:{"current-page":10,pages:10,"base-url":`/search?page=`}},Y={args:{"current-page":15,pages:50,"visible-pages":5,"base-url":`/search?page=`}},X={args:{"current-page":1,pages:1,"base-url":`/search?page=`}},K.parameters={...K.parameters,docs:{...K.parameters?.docs,source:{originalSource:`{
  args: {
    'current-page': 1,
    pages: 5,
