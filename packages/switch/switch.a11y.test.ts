...
describe("w-switch accessibility (WCAG 2.2)", () => {
	describe("axe-core automated checks", () => {
		test("default state has no violations", async () => {
			const page = render(
				html`<w-switch aria-label="Enable notifications"></w-switch>`,
			);
		...
		test("checked state has no violations", async () => {
			const page = render(
				html`<w-switch checked aria-label="Enable notifications"></w-switch>`,
			);
			await expect(page).toHaveNoAxeViolations(axeOptions);
		});
		...
		test("disabled state has no violations", async () => {
			const page = render(
				html`<w-switch disabled aria-label="Enable notifications"></w-switch>`,
			);
			await expect(page).toHaveNoAxeViolations(axeOptions);
		});
	});
	...
	describe("WCAG 4.1.2 - Name, Role, Value", () => {
		test("switch exposes role and checked state", async () => {
			...
			const wSwitch = page.container.querySelector(
				"w-switch",
			) as HTMLElement & {
				checked: boolean;
