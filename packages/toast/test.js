...
test('Multiple initializations adds only one toast container to the dom', async (t) => {
  ...
  const result = await page.evaluate(
    "document.querySelectorAll('w-toast-container').length;",
  );
  t.equal(result, 1);
});
...
test('set method: throws when no id provided', async (t) => {
  ...
  try {
    await page.evaluate(
      "customElements.get('w-toast-container').init().set();",
    );
  } catch (err) {
    error = err;
  }
...
test('set method: does not throw when an id is provided', async (t) => {
  ...
  try {
    await page.evaluate(
      "customElements.get('w-toast-container').init().set({ id: 'abc' });",
    );
  } catch (err) {
    error = err;
  }
...
test('get method: throws when no id is provided', async (t) => {
  ...
  try {
    await page.evaluate(
      "customElements.get('w-toast-container').init().get();",
    );
  } catch (err) {
    error = err;
  }
  ...
  t.match(
    error.message,
    'undefined "id" given when attempting to retrieve toast',
  );
});
...
test('del method, throws when no id provided', async (t) => {
  ...
  try {
    await page.evaluate(
      "customElements.get('w-toast-container').init().del();",
    );
  } catch (err) {
    error = err;
  }
...
test('del method, returns false when valid id does not exist', async (t) => {
  ...
  const result = await page.evaluate(
    "customElements.get('w-toast-container').init().del('abc');",
  );
...
test('updating toast type', async (t) => {
  ...
  const type = await locator.evaluate(
    (el) => el.renderRoot.querySelector('w-toast').type,
  );
...
test('Setting all properties', async (t) => {
  ...
  const page = await addContentToPage({
    page: t.context.page,
    content:
      '<w-toast id="abc" type="success" canclose text="This is my toast"></w-toast>',
  });
  ...
  t.equal(
    await locator.evaluate((el) => el.getAttribute('text')),
    'This is my toast',
  );
});
...
test('Id assigned when not provided', async (t) => {
  ...
  const locator = await page.locator('w-toast');
  t.equal(
    typeof (await locator.evaluate((el) => el.getAttribute('id'))),
    'string',
  );
...
test('API: toast method: options: duration is set correctly', async (t) => {
  ...
  await t.context.page.evaluate(
    "window.toast('This is a toast', { duration: 50 })",
  );
  const countAfter = await locator.count();
...
test('API: toast method: options: canclose creates close button', async (t) => {
  ...
  await t.context.page.evaluate(
    "window.toast('This is a toast', { canclose: true })",
  );
...
test('API: toast method: options: type changes visual appearance', async (t) => {
  ...
  await t.context.page.evaluate(
    "window.toast('This is a toast', { type: 'error' })",
  );
...
test('API: update toast modifies existing toast', async (t) => {
  ...
  // WHEN:
  await t.context.page.evaluate(
    "window.identifier = window.toast('This is a toast')",
  );
  await t.context.page.evaluate(
