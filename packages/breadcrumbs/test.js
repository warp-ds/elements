...
test("Breadcrumb component renders on the page", async (t) => {
  ...
  t.equal(
    await page.innerText("nav a"),
    "Eiendom",
    "An Eiendom a tag should be added to the page",
  );
});
...
test("Breadcrumb component interleaves / characters between breadcrumb items", async (t) => {
  ...
  t.equal(
    await page.innerText("w-breadcrumbs span"),
    "/",
    "Divider slashes should be added",
  );
});
...
test("Breadcrumb component with anchor child elements", async (t) => {
  ...
  t.equal(
    await page.locator("w-breadcrumbs a").count(),
    3,
    "3 a tags should be present",
  );
  t.equal(
    await page.locator("w-breadcrumbs span").count(),
    2,
    "2 span tags should be present",
  );
  t.match(
...
test("Breadcrumb component with last element as a span", async (t) => {
  ...
  t.equal(
    await page.locator("w-breadcrumbs a").count(),
    2,
    "2 child a tags should be present",
  );
  t.equal(
    await page.locator("w-breadcrumbs span").count(),
    3,
