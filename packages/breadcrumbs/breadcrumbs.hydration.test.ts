...
describe("w-breadcrumbs React SSR hydration", () => {
	...
	test.fails(
		"with multiple link children hydrates without warnings",
		async () => {
			// This tests the interleaving behavior - "/" separators are added between children
			const childrenHtml =
				'<a href="#/home">Home</a><a href="#/category">Category</a>';
			const warnings = await testHydrationWithChildren(
				"w-breadcrumbs",
				{},
				childrenHtml,
	...
	test.fails("with three link children hydrates without warnings", async () => {
		...
		const childrenHtml =
			'<a href="#/home">Home</a><a href="#/category">Category</a><a href="#/page">Page</a>';
		const warnings = await testHydrationWithChildren(
			"w-breadcrumbs",
			{},
			childrenHtml,
	...
	test.fails(
		"with links and span (current page) hydrates without warnings",
		async () => {
	...
	test.fails("with span children hydrates without warnings", async () => {
		...
		const childrenHtml =
			'<span>First</span><span>Second</span><span aria-current="page">Current</span>';
		const warnings = await testHydrationWithChildren(
			"w-breadcrumbs",
			{},
			childrenHtml,
	...
	test.fails("with mixed element types hydrates without warnings", async () => {
		...
		const childrenHtml =
			'<a href="#/home">Home</a><span>Category</span><a href="#/page">Page</a>';
		const warnings = await testHydrationWithChildren(
			"w-breadcrumbs",
			{},
			childrenHtml,
	...
	test.fails(
		"with aria-label and children hydrates without warnings",
		async () => {
			const childrenHtml =
				'<a href="#/home">Home</a><a href="#/category">Category</a>';
			const warnings = await testHydrationWithChildren(
...
describe("w-breadcrumbs child element DOM stability", () => {
	test.fails(
		"should preserve children in light DOM (not move to shadow DOM)",
		async () => {
			const container = document.createElement("div");
			container.innerHTML =
				'<w-breadcrumbs><a href="#/home">Home</a><a href="#/category">Category</a></w-breadcrumbs>';
	...
	test.fails("should not add classes to light DOM span children", async () => {
		const container = document.createElement("div");
		container.innerHTML =
			"<w-breadcrumbs><span>First</span><span>Current</span></w-breadcrumbs>";
		document.body.appendChild(container);
	...
	test.fails(
		"should not insert separator elements into light DOM",
		async () => {
			const container = document.createElement("div");
			container.innerHTML =
				'<w-breadcrumbs><a href="#/home">Home</a><a href="#/category">Category</a></w-breadcrumbs>';
			...
			const separators = container.querySelectorAll(
				"w-breadcrumbs > span.select-none",
			);
	...
	test.fails(
		"should preserve original child element attributes in light DOM",
		async () => {
			const container = document.createElement("div");
			container.innerHTML =
