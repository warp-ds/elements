...
export class WarpTabs extends LitElement {
	...
	private _assignSlots = () => {
		[...this.children].forEach((child) => {
			...
			if (child.tagName === "W-TAB" && !child.getAttribute("slot"))
				child.setAttribute("slot", "tabs");
			if (child.tagName === "W-TAB-PANEL" && !child.getAttribute("slot"))
				child.setAttribute("slot", "panels");
		});
		this.requestUpdate();
	};
	...
	get tabs() {
		...
		return slottedElements.filter<WarpTab>(
			(el): el is WarpTab => el.tagName.toLowerCase() === "w-tab",
		);
	}
	...
	private get _gridClass() {
		...
		return (
			ccGridLayout[`cols${count}` as keyof typeof ccGridLayout] ||
			ccGridLayout.cols1
		);
	}
