...
describe("w-checkbox-group accessibility (WCAG 2.2)", () => {
	describe("axe-core automated checks", () => {
		...
		test("required invalid state has no violations", async () => {
			...
			const groupEl = document.querySelector("w-checkbox-group") as {
	...
	describe("WCAG 2.1.1 - Keyboard", () => {
		test("tab moves through checkboxes and space toggles", async () => {
			...
			const checkboxes = [...document.querySelectorAll("w-checkbox")] as Array<
			...
			firstCheckbox?.focus();
			await expect
				.poll(() => firstCheckbox?.shadowRoot?.activeElement?.tagName)
				.toBe("INPUT");
			...
			await expect
				.poll(() => secondCheckbox?.shadowRoot?.activeElement?.tagName)
				.toBe("INPUT");
			await userEvent.keyboard("[Space]");
