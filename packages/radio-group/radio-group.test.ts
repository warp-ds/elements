...
test("disabled group renders help text in disabled color", async () => {
	...
	const helpText = group.shadowRoot?.querySelector(
		'[part~="help-text"]',
	) as HTMLElement | null;
...
test("warns when used in a form without a name", async () => {
	...
	expect(warn).toHaveBeenCalledWith(
		'w-radio-group: "name" is required for form submission.',
	);
...
test("applies group name to child radios when missing", async () => {
	...
	const radios = [...document.querySelectorAll("w-radio")] as HTMLElement[];
