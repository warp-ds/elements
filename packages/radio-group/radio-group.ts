...
export class WarpRadioGroup extends FormControlMixin(LitElement) {
	...
	private handleRadioClick = (e: Event) => {
		const clickedRadio = (e.target as HTMLElement).closest<WarpRadio>(
			"w-radio",
		);
	...
	private selectSingleRadio(
		selected: WarpRadio,
		radios = this.getAllRadios(),
	): void {
	...
	private syncTabOrder(radios: WarpRadio[]) {
		...
		if (enabledRadios.length > 0) {
			if (checkedRadio) {
				enabledRadios.forEach(
					(radio) => (radio._groupTabIndex = radio.checked ? 0 : -1),
				);
			} else {
				enabledRadios.forEach(
					(radio, index) => (radio._groupTabIndex = index === 0 ? 0 : -1),
				);
			}
		}
		...
		radios
			...
			.forEach((radio) => (radio._groupTabIndex = -1));
	}
	...
	private syncSlottedHelpText() {
		const slottedHelpText =
			this.querySelector<HTMLElement>('[slot="help-text"]');
