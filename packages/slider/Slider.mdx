...
```html
<w-slider label="Apartment size" min="0" max="250" suffix="m²">
  <w-slider-thumb
    slot="from"
...
Because of the way range sliders are arranged to avoid collision, these visual markers will only make sense for single sliders. For range sliders one of the two slider thumbs will always miss the mark.
...
<Canvas of={SliderStories.CustomError} />
