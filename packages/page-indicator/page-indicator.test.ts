...
test("marks the selected dot with correct class", async () => {
	...
	await expect
		.poll(() => {
			...
			const selectedDots = element?.shadowRoot?.querySelectorAll(
...
test("updates when selected-page changes", async () => {
	...
	expect(dots?.[0]?.classList.contains("w-page-indicator--selecteddot")).toBe(
		true,
	);
	...
	expect(dots?.[2]?.classList.contains("w-page-indicator--selecteddot")).toBe(
		true,
	);
...
test("defaults to page-count=1 and selected-page=1 when no attributes are set", async () => {
	...
	const element = page.container.querySelector(
		"w-page-indicator",
	) as HTMLElement;
