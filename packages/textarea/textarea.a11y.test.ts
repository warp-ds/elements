...
describe("w-textarea accessibility (WCAG 2.2)", () => {
	describe("axe-core automated checks", () => {
		...
		test("disabled state has no violations", async () => {
			const page = render(
				html`<w-textarea label="Message" disabled></w-textarea>`,
			);
			await expect(page).toHaveNoAxeViolations();
		});
		...
		test("required state has no violations", async () => {
			const page = render(
				html`<w-textarea label="Message" required></w-textarea>`,
			);
			await expect(page).toHaveNoAxeViolations();
		});
	...
	describe("WCAG 3.3.1 - Error Identification", () => {
		test("error state is indicated via aria-invalid", async () => {
			const page = render(
				html`<w-textarea label="Message" invalid></w-textarea>`,
			);
			await expect
	...
	describe("WCAG 4.1.2 - Name, Role, Value", () => {
		test("required state is exposed", async () => {
			const page = render(
				html`<w-textarea label="Name" required></w-textarea>`,
			);
		...
		test("disabled state is exposed", async () => {
			const page = render(
				html`<w-textarea label="Name" disabled></w-textarea>`,
			);
		...
		test("readonly state is exposed", async () => {
			const page = render(
				html`<w-textarea label="Name" readonly></w-textarea>`,
			);
			await expect
				.element(page.getByLabelText("Name"))
		...
		test("value is exposed", async () => {
			const page = render(
				html`<w-textarea label="Name" value="Hello"></w-textarea>`,
			);
