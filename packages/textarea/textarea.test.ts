...
test('validates minlength and shows error when value is too short', async () => {
  const page = render(html`<w-textarea label="Message" name="message" minlength="10"></w-textarea>`);
...
test('validates minlength passes when value is long enough', async () => {
  const page = render(html`<w-textarea label="Message" name="message" minlength="5"></w-textarea>`);
  ...
  // Type a value that meets minlength
  const textarea = page.getByLabelText('Message');
...
test('maxlength prevents typing beyond limit', async () => {
  const page = render(html`<w-textarea label="Message" name="message" maxlength="5"></w-textarea>`);
  ...
  // Native maxlength should prevent exceeding the limit
  expect(wTextArea.value.length).toBeLessThanOrEqual(5);
});

test('validates maxlength for programmatically set values', async () => {
  render(html`<w-textarea label="Message" name="message" maxlength="5"></w-textarea>`);
  ...
  // Programmatically set a value that exceeds maxlength
  wTextArea.value = 'This is way too long';
  await wTextArea.updateComplete;
