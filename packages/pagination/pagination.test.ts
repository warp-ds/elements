...
test('current page is the active page', async () => {
  ...
  await expect.element(screen.getByLabelText('Page 5')).toBeInTheDocument();
...
test('limits the number of displayed pages', async () => {
  ...
  await expect.poll(() => screen.getByRole('link').and(page.getByLabelText('Page ')).all()).toHaveLength(5);
});
...
test('shows link to first page if current page is page 3 or beyond', async () => {
  ...
  await expect.poll(() => screen.getByText('First page')).toBeInTheDocument();
});
...
test('does not show link to first page if current page is page 1 or 2', async () => {
  for (let i = 1; i <= 2; i++) {
    ...
    await expect.poll(() => screen.getByText('First page').query()).not.toBeInTheDocument();
  }
});
...
test('shows link to previous page if current page is page 2 or beyond', async () => {
  ...
  await expect.poll(() => screen.getByText('Previous page')).toBeInTheDocument();
});
...
test('does not show link to previous page if current page is the first page', async () => {
  ...
  await expect.poll(() => screen.getByText('Previous page').query()).not.toBeInTheDocument();
});
...
test('shows link to next page', async () => {
  ...
  await expect.poll(() => screen.getByText('Next page').query()).toBeInTheDocument();
});
