...
export class WarpRadio extends FormControlMixin(LitElement) {
	...
	private handleKeyDown = (event: KeyboardEvent) => {
		...
		if (
			...
			const radios = this.getStandaloneNamedRadios().filter(
	...
	private getStandaloneNamedRadios(): WarpRadio[] {
		...
		return [
			...scope.querySelectorAll<WarpRadio>(`w-radio[name="${this.name}"]`),
		].filter((radio) => !radio.closest("w-radio-group"));
	}
