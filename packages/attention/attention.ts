...
/* eslint-disable-next-line @typescript-eslint/no-unused-vars */ // Used by UnoCSS
...
const ccAttention = {
	...
	tooltip:
		"s-bg-inverted border-[--w-s-color-background-inverted] shadow-m s-text-inverted rounded-4 py-6 px-8",
	callout:
...
class WarpAttention extends LitElement {
	...
	connectedCallback() {
		...
		if (
			...
			!this.fallbackPlacements.every((fallbackPlacement) =>
				directions.includes(fallbackPlacement),
			)
		) {
			throw new Error(
	...
	get _arrowHtml(): TemplateResult | typeof nothing {
		return this.noArrow
			? nothing
			: html`<div id="arrow" class="${this._arrowClasses}"></div>`;
	}

	/** @internal */
	get _activeVariantClasses(): { wrapper: string; arrow: string } {
		if (this.callout)
			return { wrapper: ccAttention.callout, arrow: ccAttention.arrowCallout };
		if (this.popover)
			return { wrapper: ccAttention.popover, arrow: ccAttention.arrowPopover };
		if (this.tooltip)
			return { wrapper: ccAttention.tooltip, arrow: ccAttention.arrowTooltip };
		if (this.highlight)
			return {
	...
	get _targetEl(): HTMLElement | null {
		const targetSlot: HTMLSlotElement | null = this.renderRoot?.querySelector(
			"slot[name='target']",
		);
	...
	get _messageEl(): HTMLElement | null {
		const messageSlot: HTMLSlotElement | null = this.renderRoot.querySelector(
			"slot[name='message']",
		);
