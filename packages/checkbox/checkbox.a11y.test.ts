...
describe("w-checkbox accessibility (WCAG 2.2)", () => {
	describe("axe-core automated checks", () => {
		...
		test("indeterminate state has no violations", async () => {
			const page = render(
				html`<w-checkbox indeterminate>Check me</w-checkbox>`,
			);
			await expect(page).toHaveNoAxeViolations();
		});
	});
	...
	describe("WCAG 1.3.1 - Info and Relationships", () => {
		test("checkbox has accessible name from label slot", async () => {
			...
			await expect
				.element(page.getByRole("checkbox", { name: "Accept terms" }))
				.toBeVisible();
		});
	});
	...
	describe("WCAG 4.1.2 - Name, Role, Value", () => {
		...
		test("checked state is exposed", async () => {
			...
			await expect
				.element(page.getByRole("checkbox", { name: "Checked" }))
				.toBeChecked();
		});
	});
	...
	describe("WCAG 2.1.1 - Keyboard", () => {
		test("checkbox is focusable", async () => {
			...
			await expect
				.element(page.getByRole("checkbox", { name: "Focusable" }))
				.toBeVisible();
			...
			wCheckbox.focus();
			await expect
				.poll(() => wCheckbox?.shadowRoot?.activeElement?.tagName)
				.toBe("INPUT");
		});
