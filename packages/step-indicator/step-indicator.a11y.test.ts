...
describe("WCAG 4.1.2 - Name, Role, Value", () => {
	test("active step is exposed via aria-current", async () => {
		...
		await expect
			.poll(() =>
				activeStep?.shadowRoot?.querySelector('[aria-current="step"]'),
			)
			.toBeTruthy();
	});
	...
	test("step status icon has an accessible name", async () => {
		...
		await expect
			.poll(() =>
				completedStep?.shadowRoot?.querySelector(
					'[aria-label="Step indicator completed circle"]',
				),
			)
			.toBeTruthy();

		const incompleteStep = document.querySelector(
			"w-step:not([completed])",
		) as HTMLElement | null;
		expect(incompleteStep).toBeTruthy();
		await expect
			.poll(() =>
				incompleteStep?.shadowRoot?.querySelector(
					'[aria-label="Step indicator completed circle"]',
				),
			)
			.toBeFalsy();
